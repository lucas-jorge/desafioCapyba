...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ['id', 'email', 'username', 'first_name', 'last_name', 'profile_image', 'email_confirmed']
        # Só leitura
        read_only_fields = ['id', 'email_confirmed']

# Serializer específico para o processo de registro
class RegisterSerializer(serializers.ModelSerializer):
    # Adiciona campos extras que não estão no modelo diretamente, mas são necessários para o registro
    ...
    class Meta:
        ...
        fields = ['username', 'email', 'first_name', 'last_name', 'password', 'password2', 'profile_image']
        extra_kwargs = {
    ...
    def validate(self, attrs):
        ...
        # (Opcional, mas recomendado) Validar se email/username já existem aqui para dar feedback antes
...
class ItemSerializer(serializers.ModelSerializer):
    ...
    # Facilita para quem consome a API não ter que fazer outra requisição pelo ID do dono
    owner_email = serializers.ReadOnlyField(source='owner.email')

    class Meta:
        model = Item
        # campos do Item que serão expostos na API
        fields = ['id', 'title', 'description', 'is_public', 'created_at', 'owner', 'owner_email']
        # campos que são definidos pelo sistema ou não devem ser enviados diretamente pelo usuário
...
class ChangePasswordSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if data['new_password1'] != data['new_password2']:
            ...
            raise serializers.ValidationError({"new_password2": "As novas senhas não coincidem."})

        # 2. (Opcional, mas recomendado) Valida a força da nova senha usando validadores do Django
        user = self.context['request'].user  # Pega o usuário a partir do contexto passado pela View
        try:
        ...
        #     raise serializers.ValidationError({"new_password1": "A nova senha deve ser diferente da senha antiga."})
